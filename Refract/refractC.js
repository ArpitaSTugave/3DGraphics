//declarations
	...
	var counter=5;
	var basicDashPatternLength=50;
	var basicDashPatternGap=7;
	var basicDash = basicDashPatternLength - basicDashPatternGap ;
	var offset=basicDashPatternLength;
	var h=0.001;
...
if( (xcr*xcr + ycr*ycr ) < 400)  // if (x1,y1) near the mouse point
{
...
else if( (xcl*xcl + ycl*ycl ) < 400) // if (xg,yg) near the mouse point
{
...
//find refracted ray's dash style
	dashFast = basicDashPatternLength - basicDashPatternGap ; 
	vld = dashFast/eta;
	ild = basicDashPatternGap/eta;
...
for(var xx=100; xx<=canvas.width-100; xx += 4)
{
	var p11= xx;

	context.beginPath();
	context.setLineDash([basicDash,basicDashPatternGap]);
	...
	rem = ((length+offset) % basicDashPatternLength);
	offsetd = rem/eta;
...
// draw the normal
	...
	context.setLineDash([0,0]);
	context.beginPath();
	context.moveTo(p111,q111+canvas.height/3);
	context.lineTo(p111,q111-canvas.height/3);
	context.stroke();


  	context.font = '25px Times';
	context.fillStyle = 'brown';
  	context.fillText("Refractive index 1", 100,canvas.height/9);
  	context.fillText("Refractive index", canvas.width-365,canvas.height/2 + 200);
  	context.fillText(eta, canvas.width-190,canvas.height/2 + 200);
  	context.font = 'italic 25px Times';
	context.fillStyle = 'red';
  	context.fillText("N", p111-22,q111-canvas.height/3.2);
	context.strokeStyle = 'black';
...
//draw movable source
	...
	if(counter == -basicDashPatternLength)
	counter=10;
	}
